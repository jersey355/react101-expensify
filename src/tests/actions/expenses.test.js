...
test('Should remove expense from firebase test db', (done) => {
...
test('Should edit expense in firebase test db', (done) => {
    const store = createMockStore(defaultAuthState);
    ...
    store.dispatch(startEditExpense(id, updates)).then(() => {
        ...
        expect(snapshot.val().amount).toBe(777);
        done();
    });
});
...
// Asynchrounous test case requires calling "done"
...
test('Should fetch expenses from firebase test db', (done) => {
    const store = createMockStore(defaultAuthState);
